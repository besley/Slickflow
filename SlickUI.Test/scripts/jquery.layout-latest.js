/**
 * @preserve
 * jquery.layout 1.3.0 - Release Candidate 30.78大
...
,	msg: function (info, popup, debugTitle, debugOpts) {
		if ($.isPlainObject(info) && window.debugData) {
			...
			if (!$l.length)
				$l = createLog();
			$l.children("ul").append('<li style="padding: 4px 10px; margin: 0; border-top: 1px solid #AED0EA;">' + info.replace(/\</g, "&lt;").replace(/\>/g, "&gt;") + '</li>');
		}
...
,	initContent = function (pane, resize) {
		...
		if ($C && $C.length) {
			...
			if (o.applyDemoStyles) {
				//$C.css( _c.content.cssDemo ); // add padding & overflow: auto to content-div(修改过的)
