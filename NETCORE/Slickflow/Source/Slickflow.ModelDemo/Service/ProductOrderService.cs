...
using Slickflow.ModelDemo.Utility;
...
namespace Slickflow.ModelDemo.Service
{
    ...
    /// </summary>
    public partial class ProductOrderService : IProductOrderService, IWfServiceRegister
    {
        ...
        public List<ProductOrderEntity> GetPaged(ProductOrderQuery query, out int count)
        {
            ...
            {
                var poDbSet = session.GetRepository<ProductOrderEntity>().GetDbSet();
                var sqlQuery = (from vw in poDbSet
                                select vw);
                if (query.Status > 0)
                {
        ...
        private int UpdateStatus(int id, short status, IDbSession session)
        {
            ...
            //            WHERE ID=@id";
            var repository = session.GetRepository<ProductOrderEntity>();
