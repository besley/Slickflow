...
var prevactivitytree = (function () {
    ...
    prevactivitytree.showPrevActivityTree = function (runner) {
        ...
        processapi.prev(runner, function (result) {
            if (result.Status === 1) {
                ...
                $.msgBox({
                    title: "Process / Prev",
                    content: kresource.getItem("prevsteptreeshowerrormsg") + result.Message,
                    type: "warn"
                });
            }
        });
    ...
    prevactivitytree.sure = function () {
        ...
        if (selectedNodes.length <= 0) {
            $.msgBox({
                title: "Process / Prev",
                content: kresource.getItem("previousstepinfowarnmsg"),
                type: "alert"
            });
            return false;
        }
        ...
        processapi.sendback(wfAppRunner, function (result) {
            if (result.Status == 1) {
                $.msgBox({
                    title: "Process / Prev",
                    content: kresource.getItem("processsendbackokmsg"),
                    type: "info"
                });
                ...
                $.msgBox({
                    title: "Process / Prev",
                    content: kresource.getItem("processsendbackerrormsg") + result.Message,
                    type: "warn"
                });
            }
        });
