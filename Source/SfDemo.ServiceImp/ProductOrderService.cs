...
namespace SfDemo.ServiceImp
{
    ...
    public partial class ProductOrderService : ServiceBase, IProductOrderService, IWfServiceRegister
    {
        ...
        public List<ProductOrderEntity> GetPaged(ProductOrderQuery query, out int count)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(query.OrderCode))
                    predicate = Predicates.Field<ProductOrderEntity>(f => f.OrderCode, Operator.Eq, query.OrderCode);

                count = DBHelper.Count<ProductOrderEntity>(conn, predicate);
                list = DBHelper.GetPaged<ProductOrderEntity>(conn, query.PageIndex, query.PageSize,
        ...
        public ProductOrderEntity SyncOrder()
        {
            ...
            DBHelper.Insert<ProductOrderEntity>(entity);
        ...
        public WfAppResult Dispatch(ProductOrderEntity entity)
        {
            ...
            try
            {
                ...
                if (runResult.Status == WfExecutedStatus.Success)
                {
                    ...
                    //业务数据处理部分，此处是简单示例...
                    UpdateStatus(entity.ID, (short)ProductOrderStatusEnum.Dispatched, session);
                    session.Commit();
        ...
        public WfAppResult Manufacture(ProductOrderEntity entity)
        {
            ...
            try
            {
                ...
                if (result.Status == WfExecutedStatus.Success)
                {
                    ...
                    //业务数据处理部分，此处是简单示例...
                    UpdateStatus(entity.ID, (short)ProductOrderStatusEnum.Manufactured, session);
                    session.Commit();
                    appResult = WfAppResult.Success();
                }
                else
        ...
        public WfAppResult QCCheck(ProductOrderEntity entity)
        {
            ...
            try
            {
                ...
                if (result.Status == WfExecutedStatus.Success)
                {
                    ...
                    //业务数据处理部分，此处是简单示例...
                    UpdateStatus(entity.ID, (short)ProductOrderStatusEnum.QCChecked, session);
                    session.Commit();
                    appResult = WfAppResult.Success();
                }
                else
        ...
        public WfAppResult Weight(ProductOrderEntity entity)
        {
            ...
            try
            {
                ...
                if (result.Status == WfExecutedStatus.Success)
                {
                    ...
                    //业务数据处理部分，此处是简单示例...
                    UpdateStatus(entity.ID, (short)ProductOrderStatusEnum.Weighted, session);
                    session.Commit();
                    appResult = WfAppResult.Success();
                }
                else
        ...
        private int UpdateStatus(int id, short status, IDbSession session)
        {
            ...
            var rows = DBHelper.Execute(session.Connection, sql,
                    new
                    {
                        status = status,
                        id = id
            ...
            string appInstanceID, string appInstanceCode = null, string remark = null)
        {
            ...
            DBHelper.Insert<AppFlowEntity>(session.Connection, flow, session.Transaction);
        }
