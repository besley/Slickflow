...
namespace Slickflow.Engine.Core.Parser
{
    ...
    internal class PreviousStepChecker : ManagerBase
    {
            ...
            foreach (var activity in previousActivityList)
            {
                ...
                if (processModel.IsMINode(activity) == true)
                {
                    ...
                    if (processModel.IsMIParallel(activity) == true)
                    {
                        ...
                        {
                            AppendNodeViewList(nodeList, activity, ai.EndedUserId, ai.EndedUserName);
                        }
                    }
                    else if (processModel.IsMISequence(activity) == true)
                    {
                        ...
                        if (activity.ActivityId != runningNode.ActivityId)
                        {
                            ...
                            {
                                AppendNodeViewList(nodeList, activity, activityInstance.EndedUserId, activityInstance.EndedUserName);
                            }
                        }
                        else
                        {
                            ...
                            {
                                AppendNodeViewList(nodeList, activity, previousAdjacentBrotherNode.EndedUserId, previousAdjacentBrotherNode.EndedUserName);
                            }
                        }
                    }
                }
                else
                {
                    if (hasGatewayPassed == true)
                    {
                        ...
                        {
                            AppendNodeViewList(nodeList, activity, a.EndedUserId, a.EndedUserName);
                        }
                    }
                    else
                    {
            ...
            out Boolean hasGatewayPassed)
        {
            ...
            if (runningNode.MainActivityInstanceId != null)
            {
            ...
            if (isOfMultipleInstanceNode == true)
            {
                ...
                var entity = GetPreviousOfMultipleInstanceNode(runningNode.MainActivityInstanceId.Value,
                    runningNode.Id,
                    runningNode.CompleteOrder.Value);
            ...
            out bool hasPassedGatewayNode)
        {
            ...
            if (isLookUpBackSource == true)
            {
                ...
                if (runningNode.MainActivityInstanceId != null && runningNode.CompleteOrder.Value == 1)
                {
                    ...
                    backSrcActivityInstanceId = runningNode.MainActivityInstanceId.Value;
                }
