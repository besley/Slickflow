...
namespace Slickflow.Engine.Core.Pattern.Event.Conditional
{
    ...
    {
        internal NodeMediatorInterConditionalGo(ActivityForwardContext forwardContext, IDbSession session)
        ...
        {
            var expression = ActivityForwardContext.CurrentActivity.TriggerDetail.Expression;
            var pvm = new ProcessVariableManager();
            var isConditionPassed = pvm.ValidateProcessVariable(session.Connection, LinkContext.FromActivityInstance.ProcessInstanceId,
                expression, session.Transaction);
            if (isConditionPassed == false)
            {
                throw new WfRuntimeException(LocalizeHelper.GetEngineMessage("nodemediatorinterconditionalgo.CheckBeingExecutedInfo.exception"));
            }
        }
