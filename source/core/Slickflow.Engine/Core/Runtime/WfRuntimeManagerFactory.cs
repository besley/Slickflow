...
namespace Slickflow.Engine.Core.Runtime
{
    ...
    internal class WfRuntimeManagerFactory
    {
            ...
            ref WfExecutedResult result)
        {
            ...
            var nextActivityTree = rmins.ProcessModel.GetFirstActivityTreeView(startActivity, runner.Conditions);
            ...
            rmins.ActivityResource = ActivityResourceFactory.Create(runner, rmins.AppRunner.NextActivityPerformers, runner.Conditions);
            ...
            ref WfExecutedResult result)
        {
            ...
            rmins.AppRunner.NextActivityPerformers = ActivityResource.CreateNextActivityPerformers(subFirstActivity.ActivityId, 
                performerList);
            rmins.ActivityResource = ActivityResourceFactory.Create(runner, rmins.AppRunner.NextActivityPerformers);
            ...
            ref WfExecutedResult result)
        {
            ...
            if (!string.IsNullOrEmpty(runningNode.AssignedUserIds)
                && runningNode.AssignedUserIds.Contains(runner.UserId.ToString()) == false)
            {
                result.Status = WfExecutedStatus.Exception;
            ...
            {
                result.Status = WfExecutedStatus.Exception;
                result.ExceptionType = WfExceptionType.RunApp_NotInRunning;
            ...
            //判断当前节点上的必填输出变量是否存在
            //Judge whether the required output variable on the current node exists
            var currentNode = processModel.GetActivity(taskView.ActivityId);

            //检查节点上的输出变量定义，并把必填的输出变量，加入到Condition对象里面去
            //Check the output variable definition on the node and add the required output variable to the Condition object
            var pvr = new ProcessVariableResolver ();
            var isVariableRequiredChecked = pvr.CheckVariableRequired(session.Connection, taskView, currentNode, runner.Conditions, session.Transaction);
            if (isVariableRequiredChecked == false)
            {
                result.Status = WfExecutedStatus.Exception;
                result.ExceptionType = WfExceptionType.RunApp_VariableRequiredFailed;
                result.Message = LocalizeHelper.GetEngineMessage("wfruntimemanagerfactory.CreateRuntimeInstanceAppRunning.variable.error");
                return rmins;
            }

            var activityResource = ActivityResourceFactory.Create(runner,
            ...
            ref WfExecutedResult result)
        {
            ...
            if (previousActivityInstance != null && previousActivityInstance.ActivityId == jumpBackActivityId)
            {
                ...
                rmins.BackwardContext.BackwardToTargetTransitionId =
                ...
                rmins.BackwardContext.BackwardTaskReceiver = WfBackwardTaskReceiver.Instance(
                    ...
                    previousActivityInstance.EndedUserId,
                    previousActivityInstance.EndedUserName);
            ...
            else
            {
                ...
                rmins.BackwardContext.BackwardToTargetTransitionId = transition != null ? transition.TransitionId : WfDefine.WF_XPDL_GATEWAY_BYPASS_GUID;

                rmins.BackwardContext.BackwardFromActivity = processModel.GetActivity(runningNode.ActivityId);
                rmins.BackwardContext.BackwardFromActivityInstance = runningNode;
                rmins.BackwardContext.BackwardTaskReceiver = WfBackwardTaskReceiver.Instance(
                    jumpBackActivityInstance.ActivityName,
                    jumpBackActivityInstance.EndedUserId,
                    jumpBackActivityInstance.EndedUserName);
            ...
            var activityResourceBack = ActivityResourceFactory.Create(rmins.AppRunner,
            ...
            ref WfExecutedResult result)
        {
            ...
            var activityResource = ActivityResourceFactory.Create(runner, runner.NextActivityPerformers);
            rmins.ActivityResource = activityResource;
            rmins.RunningActivityInstance = runningNode;
            ...
            ref WfExecutedResult result)
        {
            ...
            else
            {
                ...
                var performerList = PerformerBuilder.CreatePerformerList(runningNode.CreatedUserId, runningNode.CreatedUserName);
            ...
            sendbackOperation.ActivityResource = ActivityResourceFactory.Create(runner, rmins.AppRunner.NextActivityPerformers);
           ...
           ref WfExecutedResult result)
        {
            ...
            runner.NextActivityPerformers = NextStepUtility.CreateNextStepPerformerList(taskDone.ActivityId,
                taskDone.AssignedUserId, taskDone.AssignedUserName);
            ...
            runner.UserId = taskToDo.AssignedUserId;
            runner.UserName = taskToDo.AssignedUserName;
            ...
            ref WfExecutedResult result)
        {
            ...
            else
            {
                rmins.AppRunner.NextActivityPerformers = ActivityResource.CreateNextActivityPerformers(
                    backSrcActivityInstance.ActivityId,
                    runningActivityInstance.CreatedUserId,
                    runningActivityInstance.CreatedUserName);
            }
            rmins.ActivityResource = ActivityResourceFactory.Create(runner, rmins.AppRunner.NextActivityPerformers);
            ...
            ref WfExecutedResult result)
        {
            ...
            rmins.AppRunner.NextActivityPerformers = ActivityResource.CreateNextActivityPerformers(lastTaskActivityInstance.ActivityId,
                lastTaskActivityInstance.EndedUserId,
                lastTaskActivityInstance.EndedUserName);
            ...
            ref WfExecutedResult result)
        {
            ...
            //判断是否是当前登录用户的任务
            //Determine whether it is the task of the currently logged in user
            if (runningNode.AssignedUserIds.Contains(runner.UserId.ToString()) == false)
            {
                result.Status = WfExecutedStatus.Exception;
                result.ExceptionType = WfExceptionType.RunApp_HasNoTask;
