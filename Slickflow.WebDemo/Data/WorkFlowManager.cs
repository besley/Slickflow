...
namespace Slickflow.WebDemoV2._0.Data
{
    public class WorkFlowManager
    {
        ...
        {
            string strSql = string.Format(" SELECT i.*,h.* FROM WfProcessInstance i LEFT JOIN HrsLeave h ON i.AppInstanceID= h.ID where 1=1 {0} order by i.ID DESC ", sqlWhere);
            return SQLHelper.ExecuteDataset(strSql).Tables[0];
        }
        #endregion
        ...
        /// <returns></returns>
        public static BizAppFlowEntity GetBizAppFlowModelByAppInstanceID(int AppInstanceID)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID, AppName, AppInstanceID, ActivityName, Remark, ChangedTime, ChangedUserID, ChangedUserName  ");
            strSql.Append("  from BizAppFlow ");
